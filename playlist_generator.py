...
def get_args():
    ...
    parser.add_argument('--number', '-n', help='Number of episodes to add to play list', type=int, default=50)
...
def get_random_episodes(all_shows, n=50):
    show_episodes = dict()
    for show in all_shows.all():
        ...
        if show.title in BLACKLIST:
            logger.debug(f'GET_EPISODES: Show Blacklisted: {show.title} GUID={show.guid}')
            continue
        if show.title not in WHITELIST and len(WHITELIST) != 0:
            logger.debug(f'GET_EPISODES: Show Not Whitelisted: {show.title} GUID={show.guid}')
            continue
        if args.include_watched is True:
...
def tvdb_season_count(show, season):
    try:
        logger.debug(f'TVDB: Getting show "{show.title}" - GUID={show.guid}')
...
def skipped_missing(show, episode):
    try:
        ...
        if episode.index > 1:
            ...
            logger.debug(f'SKIP_CHECK: First Episode of First Season. {show.title} {season_num} GUID={show.guid}')
            return False
    except NotFound:
